...
export function GameClient() {
  ...
  const startNewGame = (scenario: CustomScenario) => {
    const freshGameState: GameState = {
      ...initialGameState,
      story: scenario.storyPrompt, // The scenario description is now the starting story
      playerState: { health: 100, sanity: 100 },
      inventory: [scenario.initialItems],
      skills: [scenario.character],
      gameStarted: true,
      isLoading: true, // Start loading for the first "real" turn
      choices: [], // Choices will be generated by the AI
    };
    
    setGameState(freshGameState);

    // We'll use a generic "start" action to get the first set of choices.
    const startAction = "بازی را شروع کن.";
    ...
    const processFirstTurn = async () => {
      try {
          const firstTurn = await generateNextTurn({
              gameState: {
                  ...freshGameState,
                  // We only need to send the core state to the AI
                  isGameOver: false, 
                  gameStarted: true, 
                  isLoading: false,
              },
          ...
          // The AI will respond with a continuation of the story, choices, etc.
          // We combine the initial story prompt with the AI's first response.
          const updatedGameState: GameState = {
              ...freshGameState,
              ...firstTurn,
              story: `${freshGameState.story}\n\n${firstTurn.story}`, // Combine initial prompt with first story part
              isLoading: false,
          };
